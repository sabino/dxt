...
fn commandError(err: anyerror, stderr: *Io.Writer) ExitCode {
    switch (err) {
        ...
        error.UnsupportedTestExecution => stderr.writeAll("error: build currently executes only selected DuckDB model/seed not_null/unique/accepted_values/relationships column generic tests and source not_null/unique/accepted_values column generic tests\n") catch {},
