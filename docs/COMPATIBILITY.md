...
| `--output`, `--output-keys` | Partial | `ls` supports legacy `text`, compact `json`, dbt-style `name`, `path`, and `selector` formats. `--output-keys` filters compact JSON to `unique_id`, `resource_type`, `name`, `package_name`, `source_name`, `alias`, source-only `identifier`, `path`, `original_file_path`, `config.materialized`, `config.tags`, and dxt's compact `selector` extension; full dbt node JSON and arbitrary nested-key traversal are not implemented. |
