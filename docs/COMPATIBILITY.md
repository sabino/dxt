...
| `dxt test` | Partial | Executes selected supported DuckDB generic tests and singular SQL tests against existing target relations plus selected supported dict-fixture unit tests in rollback-only DuckDB transactions, then writes `manifest.json` plus `run_results.json`; supported built-in and static custom model/seed/source column generic tests plus singular SQL tests honor `where`, `limit`, `severity`, `warn_if`, `error_if`, and `store_failures` for simple failure-count thresholds and deterministic DuckDB audit-table creation/drop. Supported static custom generic-test DuckDB execution errors write deterministic `error` run-result rows. | CSV/SQL unit-test fixtures, unit-test overrides, broader custom generic macros beyond static `{{ model }}` / `{{ column_name }}` bodies, `store_failures_as`, building parents before data tests, broader singular config parity. |
| `dxt build` | Partial | Executes root-project and installed-package CSV seeds with supported seed YAML `quote_columns` and `column_types` configs, selected DuckDB models, supported model/source/seed column generic tests, singular SQL tests, explicit table-level `column_name` built-in tests, source-target `relationships`, test-only selected supported unit tests, and mixed selected seed/model/test subsets; supported built-in and static custom model/seed/source column generic tests plus singular SQL tests honor `where`, `limit`, `severity`, `warn_if`, `error_if`, and `store_failures`; writes sanitized model/seed and supported static custom generic-test execution-error rows plus `skipped` rows for selected blocked descendants/tests while continuing selected independent supported seeds, models, and data tests after execution or data-test failures. | Full dbt queue semantics, broader seed config parity, wider tests, broader singular configs, CSV/SQL unit-test fixtures, mixed model/seed plus unit-test scheduling, built-in/singular generic-test runtime errors, full indirect-selection modes, `store_failures_as`. |
...
| Seeds | Partial | CSV discovery, root-project and installed-package DuckDB `seed` and `build` execution, supported seed YAML `quote_columns` / `column_types` configs, seed YAML column metadata, and supported root-project seed column or explicit table-level `column_name` generic tests including ref-backed and source-target relationships. | Full materialization semantics, hooks, grants, full-refresh behavior, and broader seed config parity. |
| Sources | Partial | YAML source tables; root-project `dbt_project.yml` `sources:` configs for supported relation identity and freshness fields; source/table `database` and `quoting` relation identity for database/schema/identifier; source/table freshness config inheritance; narrow source schema rendering; table `identifier` physical-name overrides; source refs; source columns; source column and explicit table-level `column_name` tests including ref-backed and source-target relationships; catalog/source freshness subset. | Metadata freshness, richer source metadata/external table behavior. |
...
| Generic tests | Partial | Manifest nodes, `compile` artifacts, and DuckDB execution for supported model, seed, and source column built-ins plus explicit table-level `column_name` built-ins, including explicit `accepted_values` `quote: false`, literal source-target `relationships`, and supported `where` / `limit` / severity threshold / `store_failures` configs. Custom model, source, and seed column tests are supported for root-project macros and installed-package macros called with `package.test_name`, when the body contains only static SQL plus `{{ model }}` and `{{ column_name }}`; supported custom tests execute through `dxt test` / `dxt build` and write pass/fail/warn/error Run Results rows. | Non-column and table-level custom generic tests, `store_failures_as`, typed scalar value artifact parity, adapter-dispatched test overrides, custom bodies needing broader Jinja/macro execution, general threshold expression evaluation. |
...
| `source()` | Partial | Literal, narrow scalar var-backed, and static loop-var sources in parse/list and compile-time relation rendering, including configured source database and source/table database/schema/identifier quote policy. |
| `config()` | Partial | Inline tags, materialized, schema, alias, literal model `enabled`, literal singular-test `enabled`, and literal singular-test `store_failures` in supported forms. |
...
| --- | --- | --- |
| `manifest.json` | Partial | Deterministic Manifest v12-shaped slice for supported resources, dependencies, source columns, tests, maps, seed `quote_columns` / `column_types` config fields, and source relation identity fields including database, identifier, relation_name, and quoting. |
| `run_results.json` | Partial | Run Results v6-shaped rows for supported model/seed/test execution, including sanitized model/seed execution `error` rows, data-test `fail` rows, selected blocked-resource `skipped` rows, and later independent success rows in the supported run/build branches. |
| `catalog.json` | Partial | Catalog v1-shaped entries for selected existing DuckDB relations, including configured source database metadata when selected source catalog rows are found. |
| `sources.json` | Partial | Sources v3-shaped freshness rows for supported source freshness queries; read as `--state` selector input for the first source-status selector slice. |
