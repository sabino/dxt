...
| `dxt build` | Partial | Executes root-project CSV seeds, selected DuckDB models, supported model/source/seed column generic tests, and mixed selected seed/model/test subsets. | Full dbt queue semantics, package seeds, seed configs, table-level seed/source tests, wider tests, singular/unit tests, skip/fail-fast, store failures. |
...
| Seeds | Partial | CSV discovery, root-project DuckDB seed build execution, seed YAML column metadata, and supported root-project seed column generic tests. | Package seed execution, seed configs, `quote_columns`, `column_types`, `dxt seed`, full materialization semantics. |
| Sources | Partial | YAML source tables, source/table freshness config inheritance, narrow source schema rendering, table `identifier` physical-name overrides, source refs, source columns, source column tests including ref-backed relationships, catalog/source freshness subset. | Table-level tests, richer relation config such as database/quoting/project-level source config, metadata freshness. |
...
| Generic tests | Partial | Manifest nodes and DuckDB execution for supported model, seed, and source column built-ins, including explicit `accepted_values` `quote: false`. | Custom macro-backed tests, singular tests, table-level tests, configs, typed scalar value artifact parity, store failures. |
