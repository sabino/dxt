...
| Singular tests | Partial | SQL files under configured `test-paths` are parsed as singular data tests, excluding `generic/` and `fixtures/`; manifest nodes omit generic-only fields, selectors support `test_type:singular` and `test_type:data`, and DuckDB `build`/`test` executes them through failure-row counting. | YAML patches/configs, severity and threshold configs, `where`, `limit`, `store_failures`, compiled artifact paths, indirect-selection parity, broader Jinja/macros. |
