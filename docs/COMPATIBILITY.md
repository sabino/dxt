...
| `dxt source freshness` | Partial | Queries selected DuckDB source tables with resolved source relation identity, `loaded_at_field`, `loaded_at_query`, and freshness settings from supported project-level and YAML source/table configs, then writes Sources v3-shaped results. | Metadata freshness, Jinja in freshness queries beyond narrow source schema rendering, source-status selectors, concurrency, non-DuckDB adapters. |
