...
| `dxt build` | Partial | Executes root-project CSV seeds, selected DuckDB models, supported model/source column generic tests, and mixed selected seed/model/test subsets. | Full dbt queue semantics, package seeds, seed-attached tests, wider tests, source relationship/table tests, singular/unit tests, skip/fail-fast, store failures. |
...
| Generic tests | Partial | Manifest nodes and DuckDB execution for supported built-ins, including explicit `accepted_values` `quote: false`. | Custom macro-backed tests, singular tests, configs, typed scalar value artifact parity, store failures. |
