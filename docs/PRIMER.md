...
| Public fixture gates | `scripts/check_jaffle_shop_duckdb_*.py` | Validate current public Jaffle-style parse/build subsets; CI runs the parse gate and DuckDB build gate with a pinned, checksum-verified DuckDB CLI, while local runs can target the relevant script. |
