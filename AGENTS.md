...
- During autonomous Agent OS runs, treat the main tmux Codex session as the supervisor/orchestrator: inspect state, select issues, launch/monitor workers, merge/publish green PRs, and fix orchestration bugs. Delegate issue implementation and review slices to worker subprocesses/worktrees.
- Agent OS subprocesses need host GitHub CLI auth, SSH/keyring access, and shared Git worktree metadata writes, so the repo-local orchestrator launches them with `danger-full-access`. Use it only for this trusted repository and keep role behavior constrained by issue scope, branch ownership, and validation.
