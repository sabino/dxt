...
- `dxt ls` now lists dbt-selectable resources from the same parser graph with stable text/JSON output and basic name, tag, path, `resource_type:`, `test_type:generic`, config materialization, comma intersection, whitespace union, leading/trailing `+` graph expansion, and exact exclude filters; macros are emitted in artifacts but not exposed as `ls` resources.
...
- The current M1 manual gate parses the public Jaffle Shop DuckDB project into a partial manifest with SQL models, CSV seeds, docs blocks, project macros, project `+docs.node_color` config, model/test `refs` and `sources` artifact fields, supported generic test nodes including the Jaffle `accepted_values` and `relationships` tests, and whitespace-union selector behavior for `dxt ls` when passed as one selector string. Remaining selector work includes dbt-style multi-argv selector lists and repeated `--select`/`--exclude` aggregation, plus later package-aware selector/ref coverage and deeper Jaffle artifact parity.
