...
column-level `not_null`/`unique` generic tests, writes one Run Results v6-shaped
artifact, and returns exit code `1` on test failure. It does not add seed+model
or seed+model+test DAG scheduling, wider tests, selector semantic changes,
materialization macro execution, hooks, grants, docs persistence, catalog
introspection, threaded scheduling, or partial/failed model run-results.
...
- `dxt build` has started the M3 DuckDB execution path for root-project CSV seed-only selections, selected DuckDB SQL models with `table` and `view` materializations, selected model+generic-test builds without seeds, and test-only selected DuckDB column-level `not_null`/`unique` generic tests. It loads and resolves the same Zig parser graph, applies supported selectors/excludes, writes `manifest.json`, loads seeds through the Zig-owned DuckDB CLI backend, executes selected models in dependency order, executes supported generic tests against built or already-existing attached relations, writes a minimal dbt-shaped `run_results.json` v6 slice, and returns exit code `1` when any selected generic test fails. Seed+model DAG scheduling, seed+model+test DAG scheduling, package seeds, seed configs, wider generic tests, singular tests, unit tests, source tests, custom test configs, hooks, grants, docs persistence, full-refresh semantics, `store_failures`, partial/failed model run-results, and adapter materialization macro execution remain explicit boundaries.
