...
- M1A has started with behavior-preserving `src/project/types.zig` and `src/project/selector.zig` extractions. `src/project.zig` remains the public parser/list facade and still owns most loader, parser, resolver, and manifest code until follow-up extractions move those pieces behind focused internal modules.
