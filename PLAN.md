...
4. Broader source relation config parity: source/table `identifier`, quoting,
   database, project-level source config, and metadata freshness.
...
`freshness: null`, and expanded Manifest v12-shaped source fields. It does not
implement general Jinja in source properties, source `identifier`, database or
quoting config, project-level source configs, metadata freshness, source-status
selectors, or non-DuckDB source freshness execution.
...
and default-quoted `accepted_values` tests against already-existing source
tables. It does not add table-level source tests, source relationship tests,
generic macro execution, adapter-dispatched test overrides, singular tests, unit
tests, custom test configs, `where`, `limit`, `severity`, `warn_if`,
`error_if`, `store_failures`, or `accepted_values quote: false`.
