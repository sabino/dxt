...
state/result/source-status selectors, path normalization, bracket fnmatch
classes, and richer `ls` output formats.
...
The follow-up note
`.agent/research/m3-duckdb-source-relationships-generic-tests.md` covers the
source `relationships` extension. These slices do not add table-level source
tests, generic macro execution, adapter-dispatched test overrides, singular
tests, unit tests, custom test configs, `where`, `limit`, `severity`,
`warn_if`, `error_if`, `store_failures`, or native typed accepted-value
manifest scalars.
...
comments, owners, stats beyond `has_stats`, non-DuckDB adapters, `docs serve`,
or embedded `libduckdb`.
...
- Ephemeral model.
- Generic tests: model column `unique`, `not_null`, default-quoted and explicit
  `quote: false` `accepted_values`, and ref-backed `relationships` are started;
  source column `not_null`, `unique`, and default-quoted or explicit
  `quote: false` `accepted_values`, and ref-backed `relationships` are
  started; root-project CSV seed column `not_null`, `unique`, default-quoted or
  explicit `quote: false` `accepted_values`, and ref-backed `relationships` are
...
- `dxt build` has started the M3 DuckDB execution path for root-project CSV seed-only selections, selected DuckDB SQL models with `table` and `view` materializations, selected root-project seed+model builds, selected seed+model+supported-generic-test builds, selected seed+test builds for root-project CSV seed column generic tests, selected model+generic-test builds without seeds, test-only selected DuckDB model column `not_null`/`unique`/default-quoted or explicit `quote: false` `accepted_values`/ref-backed `relationships` generic tests, source+test selected DuckDB source column `not_null`/`unique`/default-quoted or explicit `quote: false` `accepted_values`/ref-backed `relationships` generic tests, and selected seed+test DuckDB seed column `not_null`/`unique`/default-quoted or explicit `quote: false` `accepted_values`/ref-backed `relationships` generic tests. It loads and resolves the same Zig parser graph, applies supported selectors/excludes, writes `manifest.json`, loads seeds through the Zig-owned DuckDB CLI backend, executes selected seed/model nodes in dependency order for selected seed/model dependencies, executes supported generic tests against built or already-existing attached/source/target relations, writes a minimal dbt-shaped `run_results.json` v6 slice, and returns exit code `1` when any selected generic test fails. Selecting read-only unit-test resources writes a valid manifest and returns a clear unsupported-execution error without writing unit-test run results. Package seeds, seed configs, table-level seed tests, table-level source tests, wider generic tests, singular tests, unit-test execution, typed scalar accepted-value artifact parity, custom test configs, hooks, grants, docs persistence, full-refresh semantics, `store_failures`, full dbt queue interleaving, skip/fail-fast semantics, partial/failed model run-results, and adapter materialization macro execution remain explicit boundaries.
