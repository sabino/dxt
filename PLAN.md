...
- M1A has started with behavior-preserving `src/project/types.zig`, `src/project/selector.zig`, `src/project/manifest.zig`, `src/project/util.zig`, `src/project/config.zig`, and `src/project/fs.zig` extractions. `src/project/util.zig` owns shared display, membership, string sorting, and narrow YAML scalar/list helpers. `src/project/config.zig` owns `dbt_project.yml` loading and project path/docs config parsing. `src/project/fs.zig` owns deterministic resource file discovery and Linux directory traversal helpers. `src/project.zig` remains the public parser/list facade and still owns most loader, resource parser, and resolver code until follow-up extractions move those pieces behind focused internal modules.
