...
- The current M1 manual gate parses the public Jaffle Shop DuckDB project into a partial manifest with SQL models, CSV seeds, docs blocks, project macros, project `+docs.node_color` config, and supported generic test nodes, including the Jaffle `accepted_values` and `relationships` tests. The next M1 slices should add whitespace-union selector parity, model/test `refs` and `sources` artifact fields, package-aware selector/ref coverage, and deeper Jaffle artifact parity.
