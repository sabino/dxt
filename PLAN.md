...
The next source-grounded M1/M2 slices after macro parser ownership extraction are:

1. Finish macro block compatibility beyond the preserved current surface:
   `{% materialization %}`, `{% test %}`, `{% data_test %}`, macro argument
   extraction, macro patch validation, and namespace precedence, using v1
   `MacroParser`, `MacroPatchParser`, and `MacroNamespaceBuilder` plus v2 macro
   resolution and dependency listener behavior.
...
- Before starting M2 product implementation, close or explicitly re-scope the next M1 macro-compatibility behavior gap: dbt-style macro block variants (`materialization`, `test`, `data_test`), macro argument extraction, patch validation, and namespace precedence. The current macro parser extraction is intentionally behavior-preserving for the already-supported top-level `{% macro %}` surface.
