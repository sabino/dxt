...
The next source-grounded M1/M2 slices after source/exposure parser extraction are:

1. Move macro block and macro patch ownership toward parser/resolve modules,
...
- Before starting M2 product implementation, close or explicitly re-scope the next M1/M1A structural gate: moving macro block and macro patch ownership out of `src/project.zig` while preserving current behavior and keeping `src/project/loader.zig` as the graph-loading owner.
