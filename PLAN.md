...
- M1A has started with behavior-preserving `src/project/types.zig`, `src/project/selector.zig`, `src/project/manifest.zig`, and `src/project/util.zig` extractions. `src/project/util.zig` now owns shared display, membership, string sorting, and narrow YAML scalar/list helpers needed by future config/parser splits. `src/project.zig` remains the public parser/list facade and still owns most loader, parser, and resolver code until follow-up extractions move those pieces behind focused internal modules.
