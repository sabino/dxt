...
- The current M1 manual gate parses the public Jaffle Shop DuckDB project into a partial manifest with SQL models, CSV seeds, docs blocks, project macros, project `+docs.node_color` config, model/test `refs` and `sources` artifact fields, supported generic test nodes including the Jaffle `accepted_values` and `relationships` tests, and selector behavior for `dxt ls` covering whitespace unions, comma intersections, graph expansion, package selectors, multi-argv selector lists, and repeated `--select`/`--exclude` flags. Remaining M1 work includes package-aware `ref` coverage and deeper Jaffle artifact parity.
