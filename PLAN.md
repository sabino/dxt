...
- `dxt ls` now lists dbt-selectable resources from the same parser graph with stable text/JSON output and basic name, tag, path, resource type, leading/trailing `+` graph expansion, and exact exclude filters; macros are emitted in artifacts but not exposed as `ls` resources.
