...
The next source-grounded M1/M2 slices after the loader extraction are:

1. Move source and exposure YAML parser ownership out of `src/project.zig`,
   using v1 `SourceParser`, `ExposureParser`, and source processing functions
   plus v2 source/exposure resolver phases.
2. Move macro block and macro patch ownership toward parser/resolve modules,
...
- Before starting M2 product implementation, close or explicitly re-scope the next M1/M1A structural gate: moving source and exposure YAML parser ownership out of `src/project.zig` while preserving current behavior and keeping `src/project/loader.zig` as the graph-loading owner.
