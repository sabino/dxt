...
Jinja in source properties, project-level source configs, metadata freshness,
source-status selectors, or non-DuckDB source
freshness execution.
...
implement project-level source config, metadata freshness, or general Jinja
rendering in source properties.
...
slice parses `where`, `limit`, `severity`, `warn_if`, and `error_if` for model,
seed, and source generic tests, emits the supported Manifest config fields,
applies `where` and `limit` to supported built-in failure-row SQL, and
classifies pass/warn/fail statuses from simple integer failure-count threshold
comparisons. It does not implement `store_failures`, custom generic-test macro
execution, adapter-dispatched generic-test overrides, unit-test execution, full
indirect-selection parity, or a general expression evaluator.
