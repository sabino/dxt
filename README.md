...
| Selectors | names/FQN, tags, paths/files, packages, resource types, sources, exposures, config materialization, wildcards, unlimited and depth-limited `+` graph expansion, and excludes in the supported subset | YAML selectors, state/defer/result/source-status selectors, `@`, richer list formats |
