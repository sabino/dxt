...
| DuckDB seed command/build execution and run results | `core/dbt/parser/seeds.py::SeedParser`; `core/dbt/artifacts/resources/v1/seed.py::SeedConfig`, `Seed`; `core/dbt/context/providers.py::load_agate_table`; `core/dbt/task/seed.py::SeedRunner`, `SeedTask`; `core/dbt/task/build.py::BuildTask.RUNNER_MAP`; `core/dbt/artifacts/schemas/run/v5/run.py::process_run_result`; `schemas/dbt/run-results/v6.json` | `crates/dbt-parser/src/resolve/resolve_seeds.rs`; `crates/dbt-loader/src/dbt_macro_assets/dbt-adapters/macros/materializations/seeds/seed.sql`; `crates/dbt-loader/src/dbt_macro_assets/dbt-adapters/macros/materializations/seeds/helpers.sql`; `crates/dbt-loader/src/dbt_macro_assets/dbt-duckdb/macros/seed.sql`; `crates/dbt-adapter/src/adapter/mod.rs::get_seed_file_path`; `crates/dbt-schemas/src/schemas/run_results.rs::RunResultOutput` | `src/root.zig` owns `dxt seed` command dispatch; `src/project/duckdb.zig` owns the first root-project CSV seed load SQL and file-path rendering; `src/project.zig` owns current seed command/build orchestration until a runner module exists; `src/project/run_results.zig` owns null compiled fields for seed results; future work must add package seed roots, seed configs, mixed build DAG scheduling, and full materialization semantics |
| DuckDB generic test execution and run results | `core/dbt/task/build.py::BuildTask.RUNNER_MAP`; `core/dbt/task/test.py::TestRunner.execute_data_test`, `build_test_run_result`; `core/dbt/parser/sources.py::SourcePatcher.construct_sources`, `get_source_tests`, `parse_source_test`; `core/dbt/parser/schema_generic_tests.py::SchemaGenericTestParser`; `core/dbt/parser/generic_test_builders.py::TestBuilder.get_synthetic_test_names`, `build_model_str`; `core/dbt/artifacts/resources/v1/generic_test.py::GenericTest`; `core/dbt/artifacts/resources/v1/generic_test.py::TestMetadata`; `core/dbt/artifacts/schemas/run/v5/run.py::process_run_result`; `schemas/dbt/run-results/v6.json` | `crates/dbt-loader/src/dbt_macro_assets/dbt-adapters/tests/generic/builtin.sql`; `crates/dbt-loader/src/dbt_macro_assets/dbt-adapters/macros/generic_test_sql/not_null.sql`; `crates/dbt-loader/src/dbt_macro_assets/dbt-adapters/macros/generic_test_sql/unique.sql`; `crates/dbt-loader/src/dbt_macro_assets/dbt-adapters/macros/generic_test_sql/accepted_values.sql`; `crates/dbt-loader/src/dbt_macro_assets/dbt-adapters/macros/generic_test_sql/relationships.sql`; `crates/dbt-loader/src/dbt_macro_assets/dbt-adapters/macros/materializations/tests/test.sql`; `crates/dbt-loader/src/dbt_macro_assets/dbt-adapters/macros/materializations/tests/helpers.sql`; `crates/dbt-parser/src/resolve/resolve_sources.rs::resolve_sources`; `crates/dbt-parser/src/resolve/resolve_seeds.rs::resolve_seeds`; `crates/dbt-parser/src/resolve/resolve_tests/persist_generic_data_tests.rs::TestableTable`, `persist_generic_data_tests`; `crates/dbt-parser/src/resolve/resolve_tests/resolve_data_tests.rs`; `crates/dbt-schemas/src/schemas/run_results.rs::ContextRunResult`; `crates/dbt-tasks-core/src/test_aggregation.rs` | `src/project/loader.zig` owns seed-path YAML discovery; `src/project/parse.zig` owns current source table, column, and table-level source test parsing; `src/project.zig` owns current model/seed property parsing, generic-test materialization, and the test-only/source+test/seed+test `build` branches until parser and runner modules own those boundaries; `src/project/manifest.zig` owns source-test `attached_node: null`, seed column/patch metadata, dependencies, and `test_metadata.kwargs.model` serialization; `src/project/duckdb.zig` owns the first direct DuckDB rendering/execution for model, seed, and source column or explicit table-level `column_name` `not_null`, `unique`, default-quoted and explicit `quote: false` `accepted_values`, plus ref-backed `relationships` for model/seed/source tests; `src/project/run_results.zig` owns `pass`/`fail` generic-test result serialization; future work must add package seed execution, macro-backed test execution, wider generic/singular/unit tests, configs, non-ref relationship targets, typed accepted-value manifest kwargs, and store-failures semantics |
...
- Current implemented command surface is `parse`, `ls`, `clean`, `compile`,
  ...
  selected DuckDB model and seed column `not_null`/`unique`/default-quoted or
  explicit `quote: false` `accepted_values`/ref-backed `relationships`
  generic tests against built or already-existing attached relations, plus
  selected DuckDB source column `not_null`/`unique`/default-quoted or explicit
  `quote: false` `accepted_values`/ref-backed `relationships` generic tests
  against existing source and target relations. Package
...
- Test-only `dxt build` execution now supports selected DuckDB model column
  `not_null`, `unique`, default-quoted or explicit `quote: false`
  `accepted_values`, and ref-backed
  ...
  generic tests, singular tests, unit tests, table-level source/seed tests,
  custom test configs, non-ref relationship targets, typed
  accepted-value manifest kwargs, and store-failures remain future work. This is documented in
