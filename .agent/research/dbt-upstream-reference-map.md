...
- Current implemented command surface is `parse`, `ls`, `compile`, `docs
  ...
  selected DuckDB column `not_null`/`unique` generic tests against built or
  already-existing attached relations. Package seeds, wider generic/singular/unit
  tests, full dbt queue parity, and full materialization semantics remain future
  work.
