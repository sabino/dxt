...
- Current implemented command surface is `parse`, `ls`, `compile`, `docs
  ...
  selected DuckDB model column `not_null`/`unique`/default-quoted or explicit
  `quote: false` `accepted_values`/ref-backed `relationships` generic tests against built or
...
- Test-only `dxt build` execution now supports selected DuckDB model column
  ...
  generic tests, singular tests, unit tests, source tests, seed-attached column
  tests, custom test configs, non-ref relationship targets, typed
